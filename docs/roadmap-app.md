...
| 4.3 | **Durchsicht/Stockkarte** (geführt, Timeline, Foto) | ✅ **LIVE** (v1.10.0; Ausbau v1.20.0) — `inspections` + View, privater Foto-Bucket (Signed-URL), Andocken an Volk-Detailseite. **v1.20.0:** geführter 3-Schritt-Wizard (handschuh-tauglich, Tap-Stepper) + optionale **Waben-für-Waben-Erfassung** (`inspections.waben` jsonb, Multi-Toggle + Trennschied + Stifte, Kennzahl-Vorbefüllung). **v1.27.0: Spracheingabe v1** (hands-free: Diktat + Kommando-Mikro je Wizard-Seite, Web Speech `de-CH` + reiner Parser/Alias-Tabelle). Offen: v2 sprachgeführter Waben-Durchgang, Offline später |
