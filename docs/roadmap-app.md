...
| Benachrichtigungs-Engine (F3) | 🟡 **App + Function live** — wartet nur noch auf Telegram-Verknüpfung | **Täglicher Telegram-Überblick** (heute fällig + überfällig). Migration **O01 live** (pg_cron/pg_net, personenbezogene RLS, Stunden-Cron mit Zeitzonen-/Doppelversand-Logik); Edge Function `taeglicher-ueberblick` + App-Seite `/benachrichtigungen` fertig. Web-Push verworfen (Service-Worker-Killer). **Erledigt 2026-07-27:** Edge Function deployt (über den MCP, ohne CLI; `verify_jwt: false` nötig, weil eigene Auth + Cron ohne Authorization-Header — Smoke-Test 3×401), App-Seite seit v1.39.0 live, 7 Deno-Tests grün. **Offen:** nur noch Bot-Token + Cron-Secret (Function-Secret **und** Vault) + Chat-ID, alles zugangsdatenbehaftet → beim Nutzer. Sofort-Alarme/Quittieren/E-Mail = spätere Zyklen. |
