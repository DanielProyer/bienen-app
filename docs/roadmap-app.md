...
| Benachrichtigungs-Engine (F3) | 🟡 **gebaut** (Deploy als v1.40.0 geplant) — wartet auf Scharfschaltung | **Täglicher Telegram-Überblick** (heute fällig + überfällig). Migration **O01 live** (pg_cron/pg_net, personenbezogene RLS, Stunden-Cron mit Zeitzonen-/Doppelversand-Logik); Edge Function `taeglicher-ueberblick` + App-Seite `/benachrichtigungen` fertig. Web-Push verworfen (Service-Worker-Killer). **Offen:** Function-Deploy + Secrets + Telegram-Verknüpfung (Rechner), dann App-Deploy. Sofort-Alarme/Quittieren/E-Mail = spätere Zyklen. |
