...
| 4.21 | Wissensdatenbank DB-gestützt (iterativ) | 🔨 **Zyklus 1 LIVE (v1.21.0):** schnelle Häppchen-Schicht (const-Katalog `wissen_katalog.dart`, 7 Durchsicht-Zeichen mit SVG-Skizzen) + generischer `key`-Deep-Link + eigene Fotos je Betrieb (`wissen_fotos`/M01, Bucket `wissen-photos`). **Zyklus 2 (v1.22.0):** Varroa (7 Einträge), ⓘ in Milbendiagnose + Behandlungs-Journal. **Zyklus 3 (v1.23.0):** Krankheiten (7 Einträge inkl. Steinbrut-Arbeitsschutz), ⓘ am Krankheits-Dropdown (reuse `krankheit.dart`). Statisch davor: v1.15.1 20 Eigen-Recherchen, v1.15.2 +8 BGD-Merkblätter. Folge: Fütterung, Honig-Ernte, Zucht, Recht. BGD-Findings: `docs/bienen-ch-findings.md` |
