...
void main() {
  test('aktiveMeldepflichtProvider: aktive meldepflichtige Ereignisse (zu_bekaempfen + neobiota)', () async {
    ...
    await fake.speichern(Gesundheitsereignis(id: '', volkId: 'v1', festgestelltAm: DateTime(2026, 7, 19), krankheit: 'vespa_velutina'));
    ...
    final aktiv = c.read(aktiveMeldepflichtProvider('v1')).map((e) => e.krankheit).toSet();
    // afb=zu_bekaempfen, vespa=neobiota → beide meldepflichtig; kalkbrut=nicht; efb=abgeschlossen
    expect(aktiv, {'afb', 'vespa_velutina'});
  });
}
