...
- **⚠️ Dieses Repo ist ÖFFENTLICH, und `assets/` wird mit der App ausgeliefert.** Alles unter `assets/` ist ohne Login über die Pages-URL abrufbar (`…/assets/assets/recherche/*.md`) — die Repo-Sichtbarkeit ist **nicht** die Grenze (D-90). Deshalb: **keine Adressen, keine privaten E-Mails/Telefonnummern** in `assets/` oder `lib/`. Region und Höhe („Arosa", „1570 m") sind erlaubt und fachlich nötig; Behörden-Kontakte ebenso. `test/pii_guard_test.dart` wacht darüber. Persönliche/betriebliche Details gehören in die **private** Imkerei-Schiene (`../imkerei`, Remote `bienen-imkerei`).
...
- **DB-Migrationen (Disziplin aus KMU Tool 2):** nummerierte SQL-Files unter `supabase/migrations/` **und** via Supabase-MCP `apply_migration`; Kopf-Kommentar, stabile errcodes (`BA0xx`), `revoke execute … from anon, **authenticated**, public` + `grant … to den wirklich nötigen Rollen` (ein Revoke von PUBLIC entzieht `authenticated` NICHT — Supabase vergibt ihm ein eigenes Grant, siehe D-84), `SET search_path = ''` + volle Qualifizierung auf DEFINER-Funktionen, Rollback-DO-SQL-Tests, `get_advisors(security)` → 0 neue Findings. Ops-Skripte (Bootstrap/Rollback) unter `supabase/ops/` (kein Migrationsfile).
