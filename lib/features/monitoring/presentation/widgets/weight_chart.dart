import 'package:fl_chart/fl_chart.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:bienen_app/core/theme/app_tokens.dart';
import 'package:bienen_app/features/monitoring/presentation/providers/monitoring_provider.dart';
import 'package:bienen_app/features/monitoring/presentation/providers/chart_data_provider.dart';
import 'package:bienen_app/shared/widgets/app_card.dart';
import 'package:bienen_app/shared/widgets/empty_state.dart';
import 'package:bienen_app/shared/widgets/section_header.dart';

class WeightChart extends ConsumerWidget {
  const WeightChart({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final weightSpots = ref.watch(weightChartDataProvider);
    final range = ref.watch(weightRangeProvider);
    final timeRange = ref.watch(timeRangeProvider);
    final timeLabels = ref.watch(chartTimeLabelProvider);

    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SectionHeader(
            titel: 'Gewichtsverlauf',
            action: _TimeRangeChips(selected: timeRange),
          ),
          const SizedBox(height: BeeTokens.md),
          if (weightSpots.isEmpty)
            const SizedBox(
              height: 200,
              child: EmptyState(
                icon: Icons.show_chart,
                titel: 'Noch keine Messdaten vorhanden',
                text: 'Daten werden angezeigt sobald die Waage verbunden ist',
              ),
            )
          else
            SizedBox(
              height: 250,
              child: LineChart(
                LineChartData(
                  gridData: FlGridData(
                    show: true,
                    drawVerticalLine: false,
                    horizontalInterval: _weightInterval(range),
                    getDrawingHorizontalLine: (value) => const FlLine(
                      color: BeeTokens.rand,
                      strokeWidth: 1,
                    ),
                  ),
                  titlesData: FlTitlesData(
                    topTitles: const AxisTitles(
                        sideTitles: SideTitles(showTitles: false)),
                    rightTitles: const AxisTitles(
                        sideTitles: SideTitles(showTitles: false)),
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: 50,
                        getTitlesWidget: (value, meta) => Padding(
                          padding: const EdgeInsets.only(right: BeeTokens.xs),
                          child: Text(
                            '${value.toStringAsFixed(0)} kg',
                            style: const TextStyle(
                              color: BeeTokens.textGedaempft,
                              fontSize: 10,
                            ),
                          ),
                        ),
                      ),
                    ),
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: 28,
                        getTitlesWidget: (value, meta) {
                          final match = timeLabels
                              .where((l) =>
                                  (l.position - value).abs() <
                                  _labelThreshold(timeRange))
                              .toList();
                          if (match.isEmpty) return const SizedBox.shrink();
                          return Padding(
                            padding: const EdgeInsets.only(top: BeeTokens.xs),
                            child: Text(
                              match.first.label,
                              style: const TextStyle(
                                color: BeeTokens.textGedaempft,
                                fontSize: 10,
                              ),
                            ),
                          );
                        },
                      ),
                    ),
                  ),
                  borderData: FlBorderData(show: false),
                  minY: range.min,
                  maxY: range.max,
                  lineBarsData: [
                    // Weight line
                    LineChartBarData(
                      spots: weightSpots,
                      isCurved: true,
                      curveSmoothness: 0.2,
                      color: BeeTokens.honig,
                      barWidth: 2.5,
                      dotData: const FlDotData(show: false),
                      belowBarData: BarAreaData(
                        show: true,
                        color: BeeTokens.honig.withAlpha(30),
                      ),
                    ),
                  ],
                  lineTouchData: LineTouchData(
                    touchTooltipData: LineTouchTooltipData(
                      getTooltipItems: (spots) => spots.map((spot) {
                        return LineTooltipItem(
                          '${spot.y.toStringAsFixed(1)} kg',
                          const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.w600,
                            fontSize: 12,
                          ),
                        );
                      }).toList(),
                    ),
                  ),
                ),
              ),
            ),
        ],
      ),
    );
  }

  double _weightInterval(({double min, double max}) range) {
    final span = range.max - range.min;
    if (span > 20) return 5;
    if (span > 10) return 2;
    return 1;
  }

  double _labelThreshold(TimeRange range) {
    switch (range) {
      case TimeRange.week:
        return 12; // ~12h tolerance
      case TimeRange.month:
        return 24;
      case TimeRange.quarter:
        return 48;
    }
  }
}

class _TimeRangeChips extends ConsumerWidget {
  final TimeRange selected;

  const _TimeRangeChips({required this.selected});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: TimeRange.values.map((range) {
        final isSelected = range == selected;
        return Padding(
          padding: const EdgeInsets.only(left: BeeTokens.xs),
          child: FilterChip(
            label: Text(_label(range)),
            selected: isSelected,
            onSelected: (_) {
              ref.read(timeRangeProvider.notifier).state = range;
              ref.read(weightReadingsProvider.notifier).refresh();
            },
            visualDensity: VisualDensity.compact,
            showCheckmark: false,
            labelStyle: TextStyle(
              fontSize: 11,
              fontWeight: FontWeight.w500,
              color: isSelected
                  ? BeeTokens.textPrimaer
                  : BeeTokens.textGedaempft,
            ),
          ),
        );
      }).toList(),
    );
  }

  String _label(TimeRange range) {
    switch (range) {
      case TimeRange.week:
        return '7T';
      case TimeRange.month:
        return '30T';
      case TimeRange.quarter:
        return '90T';
    }
  }
}
